...
start_message_text = "IF you have GUTS, then score the 100 Try if u HAVE!!!"
start_message = font.render(start_message_text, True, white)
start_time = pygame.time.get_ticks()

picture_dir = Path("C:/Users/athyr/Desktop/pictures")
enemy_images = []
...
for i in range(9):
    ...
    enemies.add(enemy)
def reset_game():
    global score, game_over, win, player, all_sprites, bullets, enemies, heart_system, start_time
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if game_over or win:
                restart_rect = pygame.Rect(WIDTH // 2 - 120, HEIGHT // 2 + 60, 200, 50)
                quit_rect = pygame.Rect(WIDTH // 2 - 120, HEIGHT // 2 + 130, 200, 50)
    ...
    if pygame.time.get_ticks() - start_time < 2500:
        screen.blit(start_message, (30, 20))
    ...
    if game_over:
        ...
        restart_button = pygame.Surface((200, 50))
        restart_button.fill((0, 180, 0))
        restart_rect = restart_button.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 80))
        screen.blit(restart_button, restart_rect)
        screen.blit(font.render("RESTART", True, white), (restart_rect.x + 40, restart_rect.y + 10))

        quit_button = pygame.Surface((200, 50))
        quit_button.fill((180, 0, 0))
        quit_rect = quit_button.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 150))
        screen.blit(quit_button, quit_rect)
        screen.blit(font.render("QUIT", True, white), (quit_rect.x + 70, quit_rect.y + 10))

    elif win:
        win_text = font.render("YAHH! YOU WIN", True, green)
